"""File is directly copied from sbi-toolbox (https://github.com/mackelab/sbi/) 
modification by OV
added behaviour log hist for posteriors
"""
...
def set_texstylefig():
    """
    Format color and fonts such that figure can be inserted into latex
...
def get_figsize(columnwidth=246., wf=0.5, hf=(5.**0.5-1.0)/2.0, ):
    """Parameters:
    - columnwidth [float]: defalault is the size for a double columt latex article (pt)
...
def add_subplot_letters(axs,fig):
    """ add letters on top of every suplot"""
    for label, ax in axs.items():
        # label physical distance to the left and up:
        trans = mtransforms.ScaledTranslation(-20/72, 7/72, fig.dpi_scale_trans)
        ax.text(0.0, 1.0, label, transform=ax.transAxes + trans,
                fontsize='large', va='bottom', fontfamily='sans')
...
def pairplot(
    ...
    **kwargs
    ...
    Args:
        ...
        upper: Plotting style for upper diagonal, {hist, log_hist,scatter, contour, cond, None, }.
        diag: Plotting style for diagonal, {hist, log_hist, cond, None}.
    ...
    def diag_func(row, **kwargs):
        if len(samples) > 0:
            for n, v in enumerate(samples):
                ...
                elif opts["diag"][n] == "log_hist": 
                    h = plt.hist(
                        v[:, row], color=opts["samples_colors"][n],log = True,
                         **opts["hist_diag"],
                    )
