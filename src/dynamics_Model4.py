...
def StochSim_o(params,torch=True):
    ...
    if torch:
        ...
        tau_noise = params['tau_noise'].numpy()
    ...
    # noise = 0
    #Precompute stuff
    dt_inv_tau_noise = dt*(1/tau_noise)
    dt_tau_w = dt/tau_w
    dt_tau = dt*(1/tau)

    x,w = run_sim_o(n,w,x,b,mu,a,theta,j,c,sigma,sqrtdt,dt_inv_tau_noise,dt_tau_w,dt_tau)
    return t,x,w

@jit(nopython=True)
def run_sim_o(n,w,x,b,mu,a,theta,j,c,sigma,sqrtdt,dt_inv_tau_noise,dt_tau_w,dt_tau):
    noise = 0
    for i in range(n - 1):
        noise = noise+ (dt_inv_tau_noise*-noise)+ (sigma*sqrtdt*np.random.normal())
        w[i+1] = w[i] + dt_tau_w*((-w[i]+b*x[i]))#+(sigma*noise[i])
        I = -a*(c*x[i]-w[i]-theta+mu) # Input 
        x[i+1] = x[i] + (dt_tau*(-x[i] +(j/ (1+np.exp(I)))+ noise))
    return x,w
