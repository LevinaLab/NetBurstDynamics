...
def smallBurstDet(xI,tI,fp_stat,minDist=100):
    ...
    # T_on = np.where(tI==T_on)
    #find OFF time
    mask = xI<fp_stat[0][2]-0.4
    T_offMask = np.diff(tI[mask])>minDist
    T_off = tI[mask][np.hstack([False,T_offMask])]-5
    # T_off = np.where(tI==T_off)
    return np.nan,T_on,T_off
...
class TransitionP(object):
    ...
    def getFeatures(self,xI,wI,tI,HMM=False,filt=True,window=801,order=5):
        ...
        if HMM:
            ...
            Z,onT_,offT_ = smallBurstDet(xI,tI,self.fp_stat,minDist=100)
            ...
            if len(onT)==len(offT):
                ibis = onT-offT
                w_diff = wI[offT]-wI[onT]
                w_ratio= wI[offT]/wI[onT]
            else:
                print('check the onT and offT')
