...
from src.dynamics_Model3 import StochSim_o
...
#     x,inverted = grid_inv(x)
    if np.any(x>=70.):#np.sum(x>thr_scale)>10000 and np.any(x<=0.):#bs=0.55 is standard
        # print('a')
        dt= np.diff(t[::100])[0]
        ...
        if len(bursts_all)>2:
            if nb>=n_events:
                ...
                for i in range(max_iter):#max number of iterations
                    ...
                    params['x0']=x[-1]
                    params['w0']=w[-1]
                    if np.any(x>=70.):#np.sum(x>7.)>10000 and np.any(x<=0.):
                        bursts = na(spike_burst_det(x[::100],dt,burst_detection_params))
                        if len(bursts)>0:
