...
from src.dynamics_Model4 import StochSim_o
...
def cut_bursts(t,x,bursts,thr=6):
    new_bursts = []
    for b in bursts:
        mask = (t>b[0])*(t<b[1])
        if np.mean(x[mask])>thr:
            new_bursts.append(b)
    return na(new_bursts)


from numba import jit
@jit
def find_ends(raw_signal,signal,burst_starts):
    burst_end = []
    burst_start_clean = []
    # burst_amp = []
    for b_start,b_end in zip(burst_starts[:-1],burst_starts[1:]):
        mv = np.max(raw_signal[b_start:b_start+50])
        if (mv>4.75):#*(mv<15.):
            burst_start_clean.append(b_start)
            burst_end.append(b_start+np.argmin(signal[b_start:b_end]))
        # burst_amp.append(np.min(signal[b_start:b_end]))
    return burst_start_clean,burst_end
    
    
    
def detect_derivat(x_dyn,dt):
    raw_signal = x_dyn[::100]
    d_sig = np.diff(raw_signal)
    thr = np.std(d_sig)*3
    # thr = np.percentile(d_sig,99)
    peaks1 = find_peaks(d_sig,height = thr,distance=20)
    burst_starts = peaks1[0]
    # burst_starts = burst_starts[:-1]
    lbs = len(burst_starts)#length burst start 
    if lbs>2:
        burst_start_clean,burst_stops = find_ends(raw_signal,d_sig,burst_starts)
        bursts = (np.vstack([burst_start_clean,burst_stops]).T)*dt
    else:
        bursts= [np.nan,np.nan]
...
#     x,inverted = grid_inv(x)
    if np.any(x>=7.)*np.any(x<0.)*np.median(x<4.):#np.sum(x>thr_scale)>10000 and np.any(x<=0.):#bs=0.55 is standard
        # print('a')
        dt= np.diff(t[::100])[0]
        bursts_all = detect_derivat(x,dt)#na(spike_burst_det(x[::100],dt,burst_detection_params))
        # bursts_all = cut_bursts(t,x,bursts_all,thr=6)
        t_last =t[-1]
        # print(t_last)
        nb = len(bursts_all)
        if len(bursts_all)>2:
            if nb>=n_events:
                ...
                for i in range(max_iter):#max number of iterations
                    ...
                    params['x0']=x[-1]
                    params['w0']=w[-1]
                    if np.any(x>=7.)*np.any(x<0.)*np.median(x<5.):
                        bursts =  na(detect_derivat(x,dt))#na(spike_burst_det(x[::100],dt,burst_detection_params))
                        # bursts = cut_bursts(t,x,bursts,thr=6)
                        if len(bursts)>0:
