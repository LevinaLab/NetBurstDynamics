...
def StochSim_o(params,torch=True):
    ...
    noise = 0
    x,w = run_sim_o(n,w,x,b,tau_w,mu,a,theta,j,c,sigma,noise,tau,dt,sqrtdt)
    return t,x,w

@jit(nopython=True)
def run_sim_o(n,w,x,b,tau_w,mu,a,theta,j,c,sigma,noise,tau,dt,sqrtdt):
    for i in range(n - 1):
        #1/50
        noise = noise+ (dt*(1/1000)*-noise)+ (sqrtdt* np.random.normal())
        w[i+1] = w[i] + dt*((-w[i]+b*x[i])/tau_w)#+(sigma*noise[i])
        I = -a*(c*x[i]-w[i]-theta+sigma*noise) # Input 
        x[i+1] = x[i] + dt*((1/tau)*(-x[i] +(j/ (1+np.exp(I))))) +mu
    return x,w
